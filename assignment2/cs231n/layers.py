...
def batchnorm_backward(dout, cache):
    ...
    N = dout.shape[0]
    dnorm_x = dout * cache['gamma']
    
    dvar = -1 / 2 * np.sum(dnorm_x * (cache['norm_x'] / (cache['den']**2)), axis=0)
    
    dmean = np.sum(dnorm_x * -1 / cache['den'], axis=0) \
            - (2 / N)  *  np.sum(dvar * (cache['norm_x'] * cache['den']), axis = 0)
    dx = dnorm_x / cache['den'] + (2 / N) * dvar * (cache['norm_x'] * cache['den']) \
         + dmean / N
